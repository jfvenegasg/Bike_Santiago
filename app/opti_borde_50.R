archivo_50_borde<-"Data/escenario_borde_50_app_latinr.RData"


#### Interfaz opti borde ####
optiborde50_UI <- function(id, label = "optiborde_50") {
  ns <- NS(id)
  
  tabItem(tabName="dash_borde_50",h2("Resultados gestor logistico con priorizacion de borde al 50%"),
          fluidRow(box(title="Mapa de la red desbalanceada",leafletOutput(ns("mapa_borde_ini_50")),width=6,closable=TRUE,collapsible=TRUE),
                          box(title="Mapa de la red balanceada",leafletOutput(ns("mapa_borde_fin_50")),width=6,closable=TRUE,collapsible=TRUE)),
          fluidRow(
            valueBox(value = h4(textOutput(ns("est_def_ini_borde_50"))), "Estaciones deficientes", icon = icon("credit-card"),width = 3,color = "danger"),
            valueBox(value = h4(textOutput(ns("est_equi_ini_borde_50"))), "Estaciones equilibradas", icon = icon("credit-card"),width = 3,color = "primary"),
            valueBox(value = h4(textOutput(ns("est_def_fin_borde_50"))), "Estaciones deficientes", icon = icon("credit-card"),width = 3,color = "danger"),
            valueBox(value = h4(textOutput(ns("est_equi_fin_borde_50"))), "Estaciones equilibradas", icon = icon("credit-card"),width = 3,color = "primary")
            #column(width = 2)
          ),
          fluidRow(
            valueBox(value = h4(textOutput(ns("est_ex_ini_borde_50"))), "Estaciones saturadas", icon = icon("credit-card"),width = 3,color = "success"),
            column(width = 3),
            #column(width = 2),
            valueBox(value = h4(textOutput(ns("est_ex_fin_borde_50"))), "Estaciones saturadas", icon = icon("credit-card"),width = 3,color = "success"),
          ...
          #                        width=4,
          #                        descriptionBlock(header=textOutput(ns("est_def_ini_borde_50")),text="Estaciones con deficiencia"),
          #                        descriptionBlock(header=textOutput(ns("est_equi_ini_borde_50")),text="Estaciones equilibradas"),
          #                        descriptionBlock(header=textOutput(ns("est_ex_ini_borde_50")),text="Estaciones con Exceso")),
          ...
          #                     width=4,
          #                     descriptionBlock(header=textOutput(ns("est_def_fin_borde_50")),text="Estaciones con deficiencia"),
          #                     descriptionBlock(header=textOutput(ns("est_equi_fin_borde_50")),text="Estaciones equilibradas"),
          #                     descriptionBlock(header=textOutput(ns("est_ex_fin_borde_50")),text="Estaciones con Exceso"))
          #                 
          # ),
           fluidRow(column(12,box(status="danger",title="Comparacion estaciones desbalanceadas y balanceadas",width=12,plotlyOutput(ns("borde_50_des_bal")))))
  )
...
optiborde50_SV <- function(input, output, session){
  ...
  output$mapa_borde_fin_50<-renderLeaflet({
    # puntos_estaciones<-read_excel("puntos_estaciones_latinr.xlsx",sheet = "Sheet1",range="c2:g22")
    puntos_estaciones<-data.frame(puntos_estaciones)
    
    load(archivo_50_borde)
    map<-leaflet() %>%
      addProviderTiles(providers$Esri.WorldStreetMap,
                       options = providerTileOptions(noWrap = TRUE))
    alfa<-0.3
  ...
  ## ESTACIONES INICIALES borde 50%
  output$est_def_ini_borde_50<-renderText({
    # puntos_estaciones<-read_excel("puntos_estaciones_latinr.xlsx",sheet = "Sheet1",range="c2:g22")
    puntos_estaciones<-data.frame(puntos_estaciones)
    alfa<-0.3
    prioridad_1<-1.05
  ...
  output$est_equi_ini_borde_50<-renderText({
    # puntos_estaciones<-read_excel("puntos_estaciones_latinr.xlsx",sheet = "Sheet1",range="c2:g22")
    puntos_estaciones<-data.frame(puntos_estaciones)
    alfa<-0.3
    prioridad_1<-1.05
  ...
  output$est_ex_ini_borde_50<-renderText({
    # puntos_estaciones<-read_excel("puntos_estaciones_latinr.xlsx",sheet = "Sheet1",range="c2:g22")
    puntos_estaciones<-data.frame(puntos_estaciones)
    alfa<-0.3
    prioridad_1<-1.05
  ...
  ## ESTACIONES FINALES borde 50%
  output$est_def_fin_borde_50<-renderText({
  ...
  output$est_equi_fin_borde_50<-renderText({
  ...
  output$est_ex_fin_borde_50<-renderText({
  ...
  output$borde_50_des_bal<-renderPlotly({
