archivo_50_centro<-"Data/escenario_centro_50_app_latinr.RData"


#### Interfaz opti centro ####
opticentro50_UI <- function(id, label = "opticentro_50") {
  ns <- NS(id)
  
  tabItem(tabName="dash_centro_50",h2("Resultados gestor logistico con priorizacion central al 50%"),
          
          fluidRow(box(title="Mapa de la red desbalanceada",leafletOutput(ns("mapa_centro_ini_50")),width=6,closable=FALSE,collapsible=FALSE),
                          box(title="Mapa de la red balanceada",leafletOutput(ns("mapa_centro_fin_50")),width=6,closable=FALSE,collapsible=FALSE)),
          fluidRow(
            valueBox(value = h4(textOutput(ns("est_def_ini_centro_50"))), "Estaciones deficientes", icon = icon("credit-card"),width = 3,color = "danger"),
            valueBox(value = h4(textOutput(ns("est_equi_ini_centro_50"))), "Estaciones equilibradas", icon = icon("credit-card"),width = 3,color = "primary"),
            valueBox(value = h4(textOutput(ns("est_def_fin_centro_50"))), "Estaciones deficientes", icon = icon("credit-card"),width = 3,color = "danger"),
            valueBox(value = h4(textOutput(ns("est_equi_fin_centro_50"))), "Estaciones equilibradas", icon = icon("credit-card"),width = 3,color = "primary")
            #column(width = 2)
          ),
          fluidRow(
            valueBox(value = h4(textOutput(ns("est_ex_ini_centro_50"))), "Estaciones saturadas", icon = icon("credit-card"),width = 3,color = "success"),
            column(width = 3),
            #column(width = 2),
            valueBox(value = h4(textOutput(ns("est_ex_fin_centro_50"))), "Estaciones saturadas", icon = icon("credit-card"),width = 3,color = "success"),
            valueBox(h4(textOutput(ns("ele_opt_3_50"))),"Distancia Total (Mts.)", icon = icon("credit-card"),width = 3,color = "info")
            #column(width = 2)
          ),
          #fluidRow(
            # box(status="danger",title="Estaciones Iniciales",
            #                      width=4,
            #                      descriptionBlock(header=textOutput(ns("est_def_ini_centro_50")),text="Estaciones con deficiencia"),
            #                      descriptionBlock(header=textOutput(ns("est_equi_ini_centro_50")),text="Estaciones equilibradas"),
            #                      descriptionBlock(header=textOutput(ns("est_ex_ini_centro_50")),text="Estaciones con Exceso")),
            #valueBox(textOutput(ns("ele_opt_3_50")), "Distancia Total (Mts.)", icon = icon("credit-card"),width = 2,color = "success"),              
            # box(title="Distancia de rebalanceo",width=4,closable=TRUE,collapsible=TRUE,status="danger",
            #                   descriptionBlock(header=textOutput(ns("ele_opt_3_50")),text="Distancia total(Mts.)")),
                          # box(status="success",title="Estaciones Optimas",
                          #     width=4,
                          #     descriptionBlock(header=textOutput(ns("est_def_fin_centro_50")),text="Estaciones con deficiencia"),
                          #     descriptionBlock(header=textOutput(ns("est_equi_fin_centro_50")),text="Estaciones equilibradas"),
                          #     descriptionBlock(header=textOutput(ns("est_ex_fin_centro_50")),text="Estaciones con Exceso"))
                          
          #),
          fluidRow(column(12,box(status="danger",title="Comparacion estaciones desbalanceadas y balanceadas",width=12,plotlyOutput(ns("centro_50_des_bal")))))
  )
...
#### Servidor opti centro ####
opticentro50_SV <- function(input, output, session){
  
  puntos_estaciones<-read_excel("Data/puntos_estaciones_latinr.xlsx",sheet = "Sheet1",range="c2:g22")
  
  #### Mapas Centro ####
  output$mapa_centro_ini_50<-renderLeaflet({
  ...
  output$mapa_centro_fin_50<-renderLeaflet({
    # puntos_estaciones<-read_excel("puntos_estaciones_latinr.xlsx",sheet = "Sheet1",range="c2:g22")
    puntos_estaciones<-data.frame(puntos_estaciones)
    
    load(archivo_50_centro)
    map<-leaflet() %>%
      addProviderTiles(providers$OpenStreetMap.DE,
                       options = providerTileOptions(noWrap = TRUE))
    alfa<-0.3
  ...
  ## ESTACIONES INICIALES CENTRO 50%
  output$est_def_ini_centro_50<-renderText({
    # puntos_estaciones<-read_excel("puntos_estaciones_latinr.xlsx",sheet = "Sheet1",range="c2:g22")
    puntos_estaciones<-data.frame(puntos_estaciones)
    alfa<-0.3
    prioridad_1<-1.05
  ...
  output$est_equi_ini_centro_50<-renderText({
    # puntos_estaciones<-read_excel("puntos_estaciones_latinr.xlsx",sheet = "Sheet1",range="c2:g22")
    puntos_estaciones<-data.frame(puntos_estaciones)
    alfa<-0.3
    prioridad_1<-1.05
  ...
  output$est_ex_ini_centro_50<-renderText({
    # puntos_estaciones<-read_excel("puntos_estaciones_latinr.xlsx",sheet = "Sheet1",range="c2:g22")
    puntos_estaciones<-data.frame(puntos_estaciones)
    alfa<-0.3
    prioridad_1<-1.05
  ...
  ## ESTACIONES FINALES CENTRO 50%
  output$est_def_fin_centro_50<-renderText({
  ...
  output$est_equi_fin_centro_50<-renderText({
  ...
  output$est_ex_fin_centro_50<-renderText({
  ...
  output$centro_50_des_bal<-renderPlotly({
  ...
  output$ele_opt_3_50<-renderText({
