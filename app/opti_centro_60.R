archivo_60_centro<-"Data/escenario_centro_60_app_latinr.RData"


#### Interfaz opti centro ####
opticentro60_UI <- function(id, label = "opticentro_60") {
  ns <- NS(id)
  
  tabItem(tabName="dash_centro_60",h2("Resultados gestor logistico con priorizacion central al 60%"),
          fluidRow(box(title="Mapa de la red desbalanceada",leafletOutput(ns("mapa_centro_ini_60")),width=6,closable=TRUE,collapsible=TRUE),
                          box(title="Mapa de la red balanceada",leafletOutput(ns("mapa_centro_fin_60")),width=6,closable=TRUE,collapsible=TRUE)),
          fluidRow(
            valueBox(value = h4(textOutput(ns("est_def_ini_centro_60"))), "Estaciones deficientes", icon = icon("credit-card"),width = 3,color = "danger"),
            valueBox(value = h4(textOutput(ns("est_equi_ini_centro_60"))), "Estaciones equilibradas", icon = icon("credit-card"),width = 3,color = "primary"),
            valueBox(value = h4(textOutput(ns("est_def_fin_centro_60"))), "Estaciones deficientes", icon = icon("credit-card"),width = 3,color = "danger"),
            valueBox(value = h4(textOutput(ns("est_equi_fin_centro_60"))), "Estaciones equilibradas", icon = icon("credit-card"),width = 3,color = "primary")
            #column(width = 2)
          ),
          fluidRow(
            valueBox(value = h4(textOutput(ns("est_ex_ini_centro_60"))), "Estaciones saturadas", icon = icon("credit-card"),width = 3,color = "success"),
            column(width = 3),
            #column(width = 2),
            valueBox(value = h4(textOutput(ns("est_ex_fin_centro_60"))), "Estaciones saturadas", icon = icon("credit-card"),width = 3,color = "success"),
  ...
  #                                width=4,
  #                                descriptionBlock(header=textOutput(ns("est_def_ini_centro_60")),text="Estaciones con deficiencia"),
  #                                descriptionBlock(header=textOutput(ns("est_equi_ini_centro_60")),text="Estaciones equilibradas"),
  #                                descriptionBlock(header=textOutput(ns("est_ex_ini_centro_60")),text="Estaciones con Exceso")),
  ...
  #                             width=4,
  #                             descriptionBlock(header=textOutput(ns("est_def_fin_centro_60")),text="Estaciones con deficiencia"),
  #                             descriptionBlock(header=textOutput(ns("est_equi_fin_centro_60")),text="Estaciones equilibradas"),
  #                             descriptionBlock(header=textOutput(ns("est_ex_fin_centro_60")),text="Estaciones con Exceso"))
  #                         
  #         ),
           fluidRow(column(12,box(status="danger",title="Comparacion estaciones desbalanceadas y balanceadas",width=12,plotlyOutput(ns("centro_60_des_bal")))))
  )
...
opticentro60_SV <- function(input, output, session){
  ...
  output$mapa_centro_fin_60<-renderLeaflet({
    # puntos_estaciones<-read_excel("puntos_estaciones_latinr.xlsx",sheet = "Sheet1",range="c2:g22")
    puntos_estaciones<-data.frame(puntos_estaciones)
    
    load(archivo_60_centro)
    map<-leaflet() %>%
      addProviderTiles(providers$Esri.WorldStreetMap,
                       options = providerTileOptions(noWrap = TRUE))
    alfa<-0.3
  ...
  ## ESTACIONES INICIALES CENTRO 60%
  output$est_def_ini_centro_60<-renderText({
    # puntos_estaciones<-read_excel("puntos_estaciones_latinr.xlsx",sheet = "Sheet1",range="c2:g22")
    puntos_estaciones<-data.frame(puntos_estaciones)
    alfa<-0.3
    prioridad_1<-1.05
  ...
  output$est_equi_ini_centro_60<-renderText({
    # puntos_estaciones<-read_excel("puntos_estaciones_latinr.xlsx",sheet = "Sheet1",range="c2:g22")
    puntos_estaciones<-data.frame(puntos_estaciones)
    alfa<-0.3
    prioridad_1<-1.05
  ...
  output$est_ex_ini_centro_60<-renderText({
    # puntos_estaciones<-read_excel("puntos_estaciones_latinr.xlsx",sheet = "Sheet1",range="c2:g22")
    puntos_estaciones<-data.frame(puntos_estaciones)
    alfa<-0.3
    prioridad_1<-1.05
    ...
    ## ESTACIONES FINALES CENTRO 60%
  output$est_def_fin_centro_60<-renderText({
  ...
  output$est_equi_fin_centro_60<-renderText({
  ...
  output$est_ex_fin_centro_60<-renderText({
  ...
  output$centro_60_des_bal<-renderPlotly({
